  ...
  module Persistence
    ...
    module ClassMethods
      ...
      def all(keyrange = ''..'', options = {})
        connection.get_range(column_family, :start => keyrange.first, :finish => keyrange.last, :count=>(options[:limit] || 100)).map {|slice| get(slice.key) }
      end
      ...
      def decode_columns_hash(attributes)
        attributes.inject(Hash.new) do |memo, (column_name, value)|
          memo[column_name.to_s] = model_attributes[column_name].decode(value)
          memo
        end
      end
